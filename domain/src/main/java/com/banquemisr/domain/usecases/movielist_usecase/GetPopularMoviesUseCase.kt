...
class GetPopularMoviesUseCase @Inject constructor(private val iMoviesRepository: IMoviesRepository):GetMovieListUseCase {
    override suspend operator fun invoke(page:Int): Flow<Resource<PageData>> {
        return iMoviesRepository.getMovieListPopular(QueryCharacters(page=page))
    }
}